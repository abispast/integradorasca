...
public class PuntosDAO extends EntitiManager<Puntos>{
    ...
    public List<Puntos> listaActivos(){
     ...
     public List<Puntos> obtenerListaAvi() {
        return persistence.getMysql().createNamedQuery("Avisos.findAll").getResultList();
    }
     
    public List<Puntos> listaPuntos(){
        return persistence.getMysql().createNamedQuery("Puntos.findByIdArea").getResultList();
    }
